...
public class AllPortsAndRtdeTest : IRobotDashboardStreamReader
{
    ...
    public AllPortsAndRtdeTest(string robotIp, ILogLogic log)
    {
        _robotIp = robotIp;
        _log = log;
    }
    ...
    public async Task StartAsync()
    {
        ...
        await ConnectTextPorts();
        await ConnectRtde();
    }

    private async Task ConnectTextPorts()
    {
    ...
    private async Task ConnectRtde()
    {
        try
        {
            _rtdeClient = new TcpClient();
            var rtdeTask = _rtdeClient.ConnectAsync(_robotIp, 30004);
    ...
    private async Task ListenToTextPort(int port, TcpClient client, CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            var raw = Encoding.ASCII.GetString(buffer, 0, bytes);
            var lines = raw.Split('\n');
